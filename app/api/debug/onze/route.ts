/** Diagnostic temporaire : vérifie le calcul du 11 sur données réelles. À SUPPRIMER. */
  ...
  return NextResponse.json(
    {
      ...
      sample: selected.slice(0, 5).map((p) => ({
        ...
        avg: p.average, m: p.matchs, g: p.goals, last5: p.averageLast5, status: p.status,
      })),
      onze: recommended.map((p) => ({ n: p.name, pos: p.position, s: p.recommendationScore })),
      capitaine: captain?.name,
      banc: (["G", "D", "M", "A"] as const).flatMap((k) => substitutes[k] ?? []).map((p) => p.name),
